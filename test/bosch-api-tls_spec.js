 ...
 *
 * Framework: Mocha + assert (built-in Node).
 */
...
describe('verifyCloudPeerCert — Bosch cloud TLS (forum #84538 regression)', () => {
    it('ACCEPTS a leaf signed by the pinned intermediate (partial-chain path, the #84538 fix)', () => {
        // authorized=false simulates Node UNABLE_TO_GET_ISSUER_CERT for the private Bosch PKI
    ...
    it('ACCEPTS any leaf whose chain validated to a trusted system root (OAuth / Let\'s Encrypt path)', () => {
        // authorized=true + hostname match → trusted regardless of the pin
        const result = verifyCloudPeerCert(
            OTHER_DER,
            true,
            undefined,
    ...
    it('REJECTS a MITM cert not signed by the pin and not chaining to a system root', () => {
        ...
        assert.ok(result.message.includes('not trusted'), `Expected "not trusted" in: ${result.message}`);
        assert.ok(result.message.includes('SELF_SIGNED_CERT_IN_CHAIN'), `Expected error code in: ${result.message}`);
    });
    ...
    it('REJECTS a hostname mismatch even on the pinned path', () => {
        ...
        assert.ok(result instanceof Error);
        assert.ok(result.message.includes('hostname'), `Expected "hostname" in: ${result.message}`);
    });
    ...
    it('REJECTS an expired certificate (now after notAfter)', () => {
        ...
        assert.ok(result instanceof Error);
        assert.ok(result.message.includes('validity window'), `Expected "validity window" in: ${result.message}`);
    });
    ...
    it('REJECTS when no peer certificate is presented', () => {
        ...
        assert.ok(result instanceof Error);
        assert.ok(result.message.includes('no peer certificate'), `Expected "no peer certificate" in: ${result.message}`);
    });
    ...
    it("default pin is the genuine Bosch 'Video CA 2A' intermediate (guards against cert swap)", () => {
        ...
        assert.ok(shipped.subject.includes('Video CA 2A'), `Expected "Video CA 2A" in subject: ${shipped.subject}`);
        assert.ok(shipped.issuer.includes('Bosch ST Root CA'), `Expected "Bosch ST Root CA" in issuer: ${shipped.issuer}`);
        // SHA-256 fingerprint pinned in CLAUDE.md / GHSA-6qh5-x5m5-vj6v
        assert.strictEqual(
            shipped.fingerprint256,
...
describe('BoschCloudAgent wiring', () => {
    it('boschCloudAgent export uses BoschCloudAgent (not a plain https.Agent)', () => {
        // Verify BoschCloudAgent is exported and is the correct class
        const agent = new BoschCloudAgent();
        assert.ok(agent instanceof BoschCloudAgent);
        // BoschCloudAgent must extend https.Agent
